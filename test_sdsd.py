...
def main():
    ...
    parser.add_argument('-opt', type=str, required=True, help='Path to SDSD test option YAML file.')
    parser.add_argument('--pretrain_model', type=str, default=None,
                        help='Optional checkpoint path to override path.pretrain_model_G.')
    parser.add_argument('--save_dir', type=str, default=None,
                        help='Optional root directory for saved SDSD inference images.')
    ...
    parser.add_argument('--no_save_gt', action='store_true',
                        help='Do not save normal-light GT frames.')
    ...
    util.setup_logger('base', opt['path']['log'], 'test_sdsd', level=logging.INFO,
                      screen=True, tofile=True)
    logger = logging.getLogger('base')
    logger.info(option.dict2str(opt))
    logger.info('Saving SDSD enhanced images to {:s}'.format(output_folder))
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        for val_data in val_loader:
            lq_path = first_item(val_data.get('LQ_path'))
            folder = first_item(val_data.get('folder'), 'scene')
            idx = first_item(val_data.get('idx'), 'frame')
            img_name = osp.splitext(osp.basename(lq_path))[0] if lq_path else str(idx).replace('/', '-')
